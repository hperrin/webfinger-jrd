...
var xrd2jrd = function (str, callback) {
  var getProperty = function (obj, Property) {
      ...
      if (Property.hasOwnProperty("@")) {
        ...
        if (
          Property["@"].hasOwnProperty("xsi:nil") &&
    ...
    res.on("end", function () {
      ...
      if (
        ...
        if (options.httpsOnly && newopts.protocol != "https:") {
          callback(
            new Error(
              "Refusing to redirect to non-HTTPS url: " + res.headers.location
            ),
            null
          );
          return;
        ...
        request(
          newopts.protocol == "https:" ? https : http,
          newopts,
          parsers,
          callback
        );
        ...
        callback(
          new Error("Bad response code: " + res.statusCode + ":" + body),
          null
        );
        return;
      }
      ...
      matched = types.filter(function (type) {
        return ct.substr(0, type.length) == type;
      });
      ...
      if (matched.length == 0) {
        callback(
          new Error(
            "Content-Type '" +
              ct +
              "' does not match any expected types: " +
              types.join(",")
          ),
          null
        );
...
var httpHostMeta = function (address, callback) {
  var options = {
    ...
    headers: {
      accept:
        "application/json, application/jrd+json, application/xrd+xml; q=0.5",
    },
  };
  ...
  request(
    ...
    options,
    {
      "application/json": jrd,
      "application/jrd+json": jrd,
      "application/xrd+xml": xrd2jrd,
    },
    callback
  );
};
...
var httpHostMetaJSON = function (address, callback) {
  ...
  request(
    http,
    options,
    { "application/json": jrd, "application/jrd+json": jrd },
    callback
  );
};
    ...
    function (err, hm) {
      ...
      json = lrdds.filter(function (link) {
        return (
          ...
          (link.type == JSONTYPE || link.type === JRDTYPE)
        );
      });
      ...
      xrd = lrdds.filter(function (link) {
        return link.hasOwnProperty("type") && link.type == XRDTYPE;
      });
      if (xrd && xrd.length > 0) {
        template(
          xrd[0].template,
      ...
      // Otherwise, give up
      throw new Error(
        "No lrdd links with templates and acceptable type in host-meta"
      );
    },
    callback
  );
